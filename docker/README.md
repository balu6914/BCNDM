...
```
peer chaincode query -C datapace_channel -n token -c '{"Args":["balanceOf","{\"user\": \"testUser\"}"]}'
```
...
```
docker exec -it cli bash
peer chaincode invoke --tls --cafile $ORDERER_CA -C datapace_channel -n fee -c '{"Args":["setFee","{\"owner\":\"<owner_id>\", \"value\": <fee_value>}"]}'
```
