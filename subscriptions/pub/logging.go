...
func (lm loggingMiddleware) PublishSubscriptionCreated(evt interface{}, toUserId string) (msgId uint64, err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method pub.PublishSubscriptionCreated(evt=%s, toUserId=%s) took %s to complete", evt, toUserId, time.Since(begin))
		if err != nil {
