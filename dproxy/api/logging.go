...
func (lm *loggingMiddleware) CreateToken(url string, ttl int) (token string, err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method create_token for url %s and ttl %d and took %s to complete", url, ttl, time.Since(begin))
		if err != nil {
	...
	return lm.svc.CreateToken(url, ttl)
}
