...
func (l loggingMiddleware) DeleteSharing(userId string, streamId string) (err error) {
	defer func(begin time.Time) {
		message := fmt.Sprintf("Method DeleteSharing(%s, %s) took %s to complete", userId, streamId, time.Since(begin))
		if err != nil {
			st, _ := status.FromError(err)
			if st != nil && st.Code() == codes.Unavailable {
				l.logger.Warn(fmt.Sprintf("Failed to unshare stream %s: sharing service is unavailable (not supported).", streamId))
				return
			}
	...
	return l.svc.DeleteSharing(userId, streamId)
}
